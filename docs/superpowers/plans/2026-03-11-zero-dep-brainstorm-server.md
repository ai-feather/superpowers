...
> **致代理工作者：** 必需：使用 superpowers:subagent-driven-development（若子代理可用）或 superpowers:executing-plans 来实现本计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**目标：** 用一个零依赖的单文件 `server.js`（仅使用 Node 内置模块）替换 brainstorm 服务器中被引入到仓库的 node_modules。

**架构：** 单文件，包含 WebSocket 协议（RFC 6455 文本帧）、HTTP 服务器（`http` 模块）和文件监听（`fs.watch`）。当作为模块被 require 时，导出协议函数以便单元测试。

**技术栈：** 仅使用 Node.js 内置模块：`http`、`crypto`、`fs`、`path`

**规格：** `docs/superpowers/specs/2026-03-11-zero-dep-brainstorm-server-design.md`
...
- **创建：** `skills/brainstorming/scripts/server.js` — 零依赖的替代实现
- **修改：** `skills/brainstorming/scripts/start-server.sh:94,100` — 将 `index.js` 改为 `server.js`
- **修改：** `.gitignore:6` — 移除 `!skills/brainstorming/scripts/node_modules/` 这条例外
...
- **无改动：** `skills/brainstorming/scripts/helper.js`、`skills/brainstorming/scripts/frame-template.html`、`skills/brainstorming/scripts/stop-server.sh`
...
服务器发出的帧永远不会带 mask。三种长度编码：
- payload < 126：2 字节 header（FIN+opcode, length）
- 126-65535：4 字节 header（FIN+opcode, 126, 16-bit length）
- &gt; 65535：10 字节 header（FIN+opcode, 127, 64-bit length）
...
客户端的帧总是带 mask。返回 `{ opcode, payload, bytesConsumed }`，不完整时返回 `null`。遇到不带 mask 的帧时抛出异常。
...
- [ ] **Step 1: 在 server.js 顶部（require 之后）添加配置和常量**
...
- [ ] **Step 2: 在模块作用域添加 WAITING_PAGE、模板加载以及辅助函数**

在模块作用域加载 `frameTemplate` 和 `helperInjection`，以便 `wrapInFrame` 和 `handleRequest` 能访问到它们。它们只从 `__dirname`（scripts 目录）读取文件，无论该模块是被 require 还是直接运行都有效。
...
- [ ] **Step 5: 添加 debounce 定时器映射**

```js
const debounceTimers = new Map();
```

文件监听逻辑内联在 `startServer`（Step 6）中，以便把 watcher 的生命周期与 server 的生命周期放在一起，并按规格要求包含一个 `error` 处理器。

- [ ] **Step 6: 添加 startServer 函数与条件式 main 入口**

`frameTemplate` 和 `helperInjection` 已经位于模块作用域（Step 2）。`startServer` 只负责创建画面目录、启动 HTTP 服务器和 watcher，并输出启动信息。
...
测试目录已有一个 `package.json`，其中 `ws` 是依赖。如有需要先安装，然后运行测试。
...
- [ ] **Step 2: 移除 .gitignore 中针对 node_modules 的例外**
